...
pub fn detect_image_type(f: &mut File) -> BlockResult<ImageType> {
    ...
    // A single aligned block is enough to sniff the QCOW2 / VHDX magic. Some
    // valid images are smaller than one block, though -- most notably a flat
    // VMDK descriptor, which is only a few hundred bytes of text. Read
...
mod unit_tests {
    ...
    fn detect_short_file_is_not_eof_error() {
        // A valid image can be smaller than one alignment block -- a flat VMDK
        // descriptor is only a few hundred bytes of text. detect_image_type
        // must read best-effort and fall through to the per-format probes
        // instead of failing with UnexpectedEof ("failed to fill whole
        // buffer").
