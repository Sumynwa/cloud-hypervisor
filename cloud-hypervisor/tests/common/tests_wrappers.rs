...
pub(crate) fn _test_virtio_block_vmdk(guest: &Guest, subformat: &str) {
    ...
    // The guest exercised the VMDK read/write paths above; make sure the
    // image is still structurally consistent afterwards.
    disk_check_consistency(vmdk_path_str, None);
}

/// Attach a small, pre-allocated flat VMDK as a *data* disk and verify:
///   1. an in-bounds direct-IO write round-trips correctly (I/O path), and
///   2. writing past the fixed virtual capacity fails with ENOSPC -- a flat
///      VMDK is pre-allocated and cannot grow like qcow2 / dynamic VHDX.
///      The image consistency is checked at the end.
///
/// Exercised for both supported flat subformats (`monolithicFlat` and
/// `twoGbMaxExtentFlat`).
pub(crate) fn _test_virtio_block_vmdk_enospc(guest: &Guest, subformat: &str) {
    // 16 MiB fixed-size data disk: small enough to fill quickly in CI.
...
/// `twoGbMaxExtentFlat` splits the virtual disk into one flat extent per 2 GiB.
/// Attach such a disk (large enough to hold >= 2 extents) as a data disk and
/// issue a direct-IO write that straddles the boundary between the first and
/// second extent. This verifies the block backend splits the request and
/// routes each half to the correct extent file (validated via a read-back
/// `cmp`), and that the image stays consistent afterwards.
///
/// `direct` mirrors the disk's `direct=on`/`off` configuration. With
/// `direct=on` the backing extents are opened with `O_DIRECT` on the host, so
/// this additionally exercises the VMDK-specific path where a single guest
/// request is split at the extent boundary and each half is bounced through an
/// `AlignedFile` to satisfy `O_DIRECT` alignment -- a path with no parallel in
/// the single-file block formats.
pub(crate) fn _test_virtio_block_vmdk_extent_spanning(guest: &Guest, direct: bool) {
    ...
    // Flat extents are sparse files, so only the straddling write below
    // actually consumes host space.
    ...
    // Parse the descriptor to find the exact guest byte offset where the first
    // extent ends (== the extent boundary), so the write below straddles it.
    // Flat extent lines look like: `RW <sectors> FLAT "name-f001.vmdk" 0`.
    ...
    let r = panic::catch_unwind(|| {
        ...
        // Write a random pattern that spans the 2 GiB extent boundary, then
        // read it back and compare. A correct backend splits the request at
        // the boundary and writes each half into the right extent file.
        guest
            .ssh_command(&format!(
